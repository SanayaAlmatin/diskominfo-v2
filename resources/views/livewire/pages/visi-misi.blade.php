@push('styles')
	<style>
		...
		.profile-visi-misi .visi-delay-1 { animation-delay: 0.08s; }
		.profile-visi-misi .visi-delay-2 { animation-delay: 0.16s; }
		.profile-visi-misi .visi-delay-3 { animation-delay: 0.24s; }
		.profile-visi-misi .visi-delay-4 { animation-delay: 0.32s; }
		.profile-visi-misi .visi-delay-5 { animation-delay: 0.4s; }
...
<main class="flex-1">
	<section class="visi-hero relative overflow-hidden text-white">
		...
		<div class="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
			<div class="visi-fade-up visi-delay-1 inline-flex items-center gap-3 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em]">
			...
			<div class="mt-6 grid gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
				<div>
					<h1 class="visi-fade-up visi-delay-2 text-3xl font-extrabold leading-tight text-white sm:text-4xl lg:text-5xl">
						Visi dan Misi
					</h1>
					...
					<nav class="visi-fade-up visi-delay-4 mt-6 flex items-center gap-2 text-xs text-blue-100" aria-label="Breadcrumb">
						<a href="{{ route('home') }}" wire:navigate class="font-semibold text-white/90 transition hover:text-white">Beranda</a>
				...
				<div class="visi-fade-up visi-delay-3">
					<div class="rounded-2xl border border-white/40 bg-white p-6 text-slate-900 shadow-2xl sm:p-8">
						...
						<div class="mt-6 flex flex-wrap gap-2">
							<span class="rounded-lg bg-[#F7D558] px-3 py-1 text-xs font-semibold text-[#044FA0]">Tangsel unggul</span>
							<span class="rounded-lg bg-[#F5F8FC] px-3 py-1 text-xs font-semibold text-[#044FA0]">Efektif dan efisien</span>
							<span class="rounded-lg bg-[#E6F2FF] px-3 py-1 text-xs font-semibold text-[#044FA0]">Kota lestari</span>
							<span class="rounded-lg bg-[#F7E9B5] px-3 py-1 text-xs font-semibold text-[#044FA0]">Saling terkoneksi</span>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="mx-auto max-w-7xl">
			...
			<div class="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
				@foreach ($missions as $index => $mission)
					...
					@endphp
					<article wire:key="mission-{{ $index }}" class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
						<div class="flex items-start gap-4">
							...
							<div>
								<p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Misi {{ $number }}</p>
								<p class="mt-2 text-base font-semibold leading-relaxed text-slate-900">{{ $mission }}</p>
							</div>
						</div>
	...
	<section class="bg-white px-4 py-12 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-7xl">
			<div class="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
				<div class="rounded-2xl bg-gradient-to-br from-[#044FA0] via-[#0C5AA6] to-[#1E78B7] p-8 text-white shadow-xl">
					...
					<a href="{{ route('home') }}" wire:navigate class="mt-6 inline-flex items-center gap-2 rounded-lg bg-[#F7D558] px-5 py-3 text-sm font-bold text-[#044FA0] transition hover:bg-white">
						Kembali ke Beranda
