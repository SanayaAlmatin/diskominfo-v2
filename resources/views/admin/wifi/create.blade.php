...
@section('title', 'Tambah Titik WiFi')
@section('page-title', 'Tambah Titik WiFi')
...
@section('content')
    <div class="max-w-4xl">
        <div class="flex items-center gap-3 mb-5">
            ...
            <h2 class="text-lg font-bold text-gray-800">Tambah Titik WiFi</h2>
        </div>

        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <form method="POST" action="{{ route('admin.wifi.store') }}" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="n_wilayah" value="{{ old('n_wilayah') }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
                    <div>
                        ...
                        <input type="text" name="kecepatan" value="{{ old('kecepatan') }}"
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3" class="tinymce-editor-simple w-full px-4 py-2.5 rounded-lg border border-gray-300 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('keterangan') border-red-400 @enderror"
                        ...
                        placeholder="Detail lokasi (contoh: Dekat pintu masuk utama)">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Add Tile Layer (Google Streets equivalent style using standard Leaflet/OSM or Mapbox)
            ...
            // Allow marker drag if already exists from validation error
            if (marker) {
                marker.on('dragend', function(e) {
                    var position = marker.getLatLng();
                    updateInputs(position.lat, position.lng);
