...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="flex items-center gap-3 mb-5">
            ...
            <h2 class="text-lg font-bold text-gray-800">Tambah Aplikasi Portal</h2>
        </div>
            ...
            <form action="{{ route('admin.aplikasi.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="url" name="href" value="{{ old('href', '#') }}" required
                            placeholder="https://..."
                ...
                <div>
                    ...
                    <textarea name="description" rows="6"
                        class="tinymce-editor-simple w-full px-4 py-2.5 rounded-lg border border-gray-300 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-400 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div x-show="iconType === 'upload'" x-cloak class="border-2 border-dashed border-gray-300 rounded-xl p-6 bg-gray-50/50 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all duration-200"
                    ...
                    <div class="flex flex-col items-center text-center">
                        ...
                        <template x-if="!logoFileName">
                            <div>
                                <p class="text-sm font-semibold text-gray-700">Klik untuk pilih file logo</p>
                                <p class="text-xs text-gray-400 mt-1">PNG, JPG, WEBP, SVG — Maks. 2MB</p>
                            </div>
                        </template>
                        <template x-if="logoFileName">
                            <div>
                        ...
                        <div x-show="logoPreviewUrl" class="mt-4">
                            <img :src="logoPreviewUrl" class="max-h-24 rounded-lg border border-gray-200 shadow-sm object-contain" alt="Preview logo">
                        </div>
                    </div>
                    @error('logo_file')
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded-xl p-3 flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0"
                                class="w-full bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-sm font-semibold text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition-all">
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('aplikasiForm', () => ({
                iconType: '{{ old('icon_type', 'material') }}',
                iconBg: '{{ old('icon_bg', 'bg-blue-100') }}',
                iconColor: '{{ old('icon_color', 'text-blue-600') }}',
                category: '{{ old('category', $categories[0]->name ?? '') }}',
                ...
                selectedIcon: '{{ old('icon_material', 'apps') }}',
                logoFileName: '',
                logoPreviewUrl: '',
                bgOptions: [
                ...
                newTag: '',
                tags: {!! json_encode(old('tags') ? json_decode(old('tags')) : []) !!},
                get tagsJson() {
