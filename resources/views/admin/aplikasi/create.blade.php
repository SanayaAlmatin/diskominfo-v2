            ...
            <form action="{{ route('admin.aplikasi.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="category" required
                            ...
                            <option value="admin" {{ old('category') == 'admin' ? 'selected' : '' }}>Administrasi</option>
                            ...
                            <option value="safety" {{ old('category') == 'safety' ? 'selected' : '' }}>Keamanan Publik
                            </option>
                        </select>
                        @error('category')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="6"
                        class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-400 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div x-show="iconType === 'upload'" class="border rounded p-4" style="display:none">
                    ...
                    <p class="mt-1 text-xs text-gray-400">Maks. 2MB. JPG, PNG, WEBP, SVG</p>
                    @error('logo_file')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
