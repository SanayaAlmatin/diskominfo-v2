...
@section('content')
    <div class="max-w-2xl">
        <div class="flex items-center gap-3 mb-5">
            ...
            <h2 class="text-lg font-bold text-gray-800">Tambah SOTK</h2>
        </div>
            ...
            <form method="POST" action="{{ route('admin.struktur-organisasi.store') }}" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="tahun" value="{{ old('tahun', date('Y')) }}" min="2000"
                            max="2100"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('tahun') border-red-400 @enderror">
                        @error('tahun')
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Nama SOTK <span
                            class="text-red-500">*</span></label>
                    <input type="text" name="nama_sotk" value="{{ old('nama_sotk') }}"
                        class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nama_sotk') border-red-400 @enderror"
                        placeholder="Contoh: Diskominfo Tangsel 2024">
                    @error('nama_sotk')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3"
                        class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('deskripsi') }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Gambar / Bagan SOTK</label>
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_current" value="1" id="is_current"
                        {{ old('is_current') ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300">
