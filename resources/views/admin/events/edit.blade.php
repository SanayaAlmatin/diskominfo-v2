...
<div class="space-y-6 max-w-7xl mx-auto">
        ...
        <div>
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Perbarui data event atau acara</p>
                </div>
            </div>
        </div>
    ...
    <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
        @method('PUT')
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $event->name) }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Label / Kategori Event <span class="text-red-500">*</span></label>
                        <input type="text" name="label" list="labelsList" value="{{ old('label', $event->label) }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="location" value="{{ old('location', $event->location) }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi Event</label>
                        <textarea name="description" rows="5" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500">{{ old('description', $event->description) }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Waktu Event</label>
                        <input type="text" name="time" value="{{ old('time', $event->time) }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    ...
                    <div class="pt-2">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="latitude" name="latitude" value="{{ old('latitude', $event->latitude ?? '-6.28862') }}" class="w-full border-gray-300 rounded-xl bg-gray-50 focus:bg-white text-sm" readonly>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Longitude</label>
                                <input type="text" id="longitude" name="longitude" value="{{ old('longitude', $event->longitude ?? '106.71789') }}" class="w-full border-gray-300 rounded-xl bg-gray-50 focus:bg-white text-sm" readonly>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pt-2">
                        ...
                        <input type="url" name="event_url" value="{{ old('event_url', $event->event_url) }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Website Penyelenggara</label>
                        <input type="url" name="website" value="{{ old('website', $event->website) }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-2 text-center hover:bg-gray-50 transition-colors relative" id="drop-zone">
                        ...
                        <div id="image-preview" class="relative mt-2">
                            @if($event->image)
                                <img id="preview-img" src="{{ Storage::url($event->image) }}" class="max-h-48 mx-auto rounded-lg shadow-sm border border-gray-200">
                            @else
                    ...
                    @if($event->image)
                        <p class="text-xs text-blue-600 mt-2 flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">info</span> Upload gambar baru untuk mengganti gambar lama.</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ old('start_date', $event->start_date ? $event->start_date->format('Y-m-d') : '') }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Tanggal Selesai</label>
                        <input type="date" name="end_date" value="{{ old('end_date', $event->end_date ? $event->end_date->format('Y-m-d') : '') }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
            </div>
...
<script>
    ...
    @if(!$event->image)
        ...
        document.getElementById('drop-zone').classList.replace('p-2', 'p-4');
    @endif
