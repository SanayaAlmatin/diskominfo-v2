...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="flex items-center gap-3 mb-5">
            ...
            <h2 class="text-lg font-bold text-gray-800">Tambah Foto Kegiatan</h2>
        </div>

        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <form method="POST" action="{{ route('admin.foto.store') }}" enctype="multipart/form-data" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="category" value="{{ old('category') }}" list="category-list"
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="tinymce-editor-simple w-full px-4 py-2.5 rounded-lg border border-gray-300 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-400 @enderror"
                        placeholder="Deskripsi singkat kegiatan (opsional)">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- Upload Foto --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Upload Foto</label>
                    <div x-data="{
                        fileName: '',
                        previewUrl: '',
                        dragging: false,
